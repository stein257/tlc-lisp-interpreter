...
(defn controlar-aridad 
  ...
  (cond
    ...
    (< (count lista) longitud) '(*error* too-few-args)
    )
  )
...
(defn nulo? 
  "Devuelve true si un elemento es nulo en TLC-LISP"
  [unElemento]
  (contains? (set '(nil 'nil NIL ())) unElemento))

(defn igual? 
  "Verifica la igualdad entre dos elementos al estilo de TLC-LISP (case-insensitive)."
  [unElemento otroElemento]
  (cond
    (nulo? unElemento) (nulo? otroElemento)
    (nulo? otroElemento) (nulo? unElemento)
    (or (string? unElemento) (string? otroElemento)) (= unElemento otroElemento)
    :else (= (lower-case unElemento) (lower-case otroElemento)) 
    ) 
  )
...
; false
 (defn error? 
   ...
   [unElemento]
   (cond
     (and (seq? unElemento) (not-empty unElemento)) (igual? '*error* (first unElemento))
     :else false 
     ) 
   )
...
(defn actualizar-amb 
  ...
  (cond
    (error? valor) amb
    (contains? (set amb) (symbol (lower-case clave))) (apply list (assoc (vec amb) (inc (.indexOf amb (symbol (lower-case clave)))) valor))
    :else (concat amb (list (symbol (lower-case clave)) valor)) 
    )
  )
...
(defn buscar
  ...
  [clave amb]
  (cond
    (empty? amb)  (list '*error* 'unbound-symbol clave)
    (igual? (first amb) clave) (second amb)
    :else (buscar clave (drop 2 amb))
    )
  )
...
(defn fnc-append 
  ...
  [lista]
  (cond
    (< (count lista) 2) '(*error* too-few-args)
    (> (count lista) 2) '(*error* too-many-args)
    (and (nulo? (nth lista 0)) (nulo? (nth lista 1))) nil
    (nulo? (nth lista 1)) (nth lista 0)
    (nulo? (nth lista 0)) (nth lista 1)
    (not (seq? (nth lista 1))) (list '*error* 'list 'expected (nth lista 1))
    (not (seq? (nth lista 0))) (list '*error* 'list 'expected (nth lista 0))
    :else (concat (nth lista 0) (nth lista 1))
    )
  )
...
(defn fnc-env 
  ...
  [res unaLista otraLista]
  (cond
    (empty? res) (concat unaLista otraLista)
...
(defn fnc-read 
  ...
  [lista]
  (cond
    (seq lista) '(*error* not-implemented) 
    :else (let [input (read)]
            (cond
              (nulo? input) nil 
              :else input
...
(defn fnc-add 
  ...
  (cond
    (< (count lista) 2) '(*error* too-few-args)
    (seq (remove number? lista)) (list '*error* 'number-expected (first (remove number? lista)))
    :else (apply + lista)
    )
  )
...
(defn fnc-sub 
  ...
  [lista]
  (cond
    (empty? lista) '(*error* too-few-args)
    (seq (remove number? lista)) (list '*error* 'number-expected (first (remove number? lista)))
    :else (apply - lista)
    )
  )
...
(defn fnc-reverse 
  ...
  [lista]
  (cond
    (< (count lista) 1) '(*error* too-few-args)
    (> (count lista) 1) '(*error* too-many-args)
    (seq (remove seq? lista)) (list '*error* 'list-expected (first (remove seq? lista)))
...
(defn evaluar-escalar 
  ...
  (cond
    (not (symbol? escalar)) (list escalar local)
    (not (error? (buscar escalar global))) (list (buscar escalar global) local) 
    (not (error? (buscar escalar local))) (list (buscar escalar local) local)
    :else (list (buscar escalar local) local)
  )
)
...
(defn evaluar-de 
  ...
  [expre amb-global _]
   (cond
    (< (count  expre) 3) (list '(*error* list expected nil) amb-global)
    (not (seq? (nth expre 2))) (list (list '*error* 'list 'expected (nth expre 2)) amb-global)
    (nil? (second expre)) (list '(*error* cannot-set nil) amb-global)
    (not (symbol? (second expre))) (list (list '*error* 'symbol 'expected (second expre)) amb-global)
    :else (list (second expre) (concat amb-global (list (second expre) (concat (list 'lambda) (drop 2 expre)))))
    )
  )
...
(defn evaluar-if 
  ...
  [expre amb-global amb-local]
  (let [eval (evaluar (second expre) amb-global amb-local)]
    (cond 
      (error? (first eval)) eval 
      (= (controlar-aridad expre 2) 2) (evaluar nil amb-global amb-local)
      (nulo? (first eval)) (evaluar-secuencia-en-cond (drop 3 expre) amb-global amb-local) 
      :else (evaluar (nth expre 2) amb-global amb-local)
      )
    )
  )
...
(defn evaluar-or 
  ...
  (let [eval (evaluar (second expre) amb-global amb-local)]
    (cond
      (<= (count expre) 2) eval
      (nulo? (first eval)) (evaluar-or (cons (first expre) (drop 2 expre)) amb-global amb-local)
      :else eval
      ) 
   )
)
...
(defn aux-partition
  "Particiona una lista de forma tal que si su longitud es impar agrega el ultimo elemento en una lista al final"
  [lista]
  (cond
    (odd? (count lista)) (concat (partition 2 lista) (list (list (last lista))))
    :else (partition 2 lista)
    )
  )

(defn agregar-setq
  "Agrega un setq al principio de una lista"
  [param]
  (concat (list 'setq) param)
  )

(defn lista-de-setq 
  "Recibe una lista de parametros separados de a 2 y devuelve una lista con setq adelante de cada elemento"
  [params]
  (map agregar-setq params)
  )
...
(defn evaluar-setq 
  ...
  (cond
    (< (count expre) 3) (list '(*error* list expected nil) amb-global)
    (nulo? (second expre)) (list '(*error* cannot-set nil) amb-global)
    (not (symbol? (second expre))) (list (list '*error* 'symbol 'expected (second expre)) amb-global)
    (> (count expre) 3) (evaluar-secuencia-en-cond (lista-de-setq (aux-partition (next expre))) amb-global amb-local)
    :else (evaluar (second expre) (actualizar-amb amb-global (second expre) (first (evaluar (last expre) amb-global amb-local))) '())
    ) 
  )
